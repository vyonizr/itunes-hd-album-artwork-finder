...
- The animated-download button is hidden below `breakpoints.tablet.max` (`src/utils/breakpoints.ts`, 768px) via a CSS media query on the button itself — same mechanism already used elsewhere in `CardAlbum/style.ts` for responsive layout, no JS user-agent sniffing needed. We deliberately use the wider `tablet.max` rather than this file's narrower `mobile.max` (480px): the goal is to gate on "resource/bandwidth-constrained touch device," and many current phones report CSS viewport widths well above 480px, which the existing `mobile` bucket alone would miss. `breakpoints.ts` itself is unchanged — this is a one-off threshold choice for this feature, not a redefinition of "mobile" app-wide. Motion artwork still autoplays in the card below this width; only the download action is unavailable, avoiding the ~25-30MB wasm fetch and heavy CPU work on constrained devices/connections.
